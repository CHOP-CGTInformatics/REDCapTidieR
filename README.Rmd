...
The `bind_tables()` function takes the output of `read_redcap_tidy`​ and bind it to an environment. By default, this is the global environment:

```{r, eval = FALSE}
# Remove eval = FALSE once bind_tables is implemented
redcap_export %>%
  bind_tables()
...
(by default, the global environment). This way you can issue the following commands to populate your environment with data from your REDCap database:
