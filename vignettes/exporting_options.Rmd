...
The goal of `write_redcap_xlsx()` is to provide a single .xlsx file where each [data tibble](glossary.html#data-tibble) (i.e., what is listed under the `redcap_data` column) is displayed in its own sheet. Let's pass the `supertbl` defined above through the function and investigate the default output:
