...
A Data Access Group (DAG) in REDCap is a feature that allows administrators to manage user permissions and restrict access to records. When a [project](#project) has DAGs enabled, a corresponding `redcap_data_access_group` [column](#column) identifies which DAG a given record belongs to. This functionality is particularly useful in multi-center studies, where each center needs to protect their data from others. DAGs enable administrators to assign user rights and maintain data confidentiality effectively.
...
The [skimr](https://docs.ropensci.org/skimr/) R package provides summary statistics to help users quickly skim and understand their data. In REDCapTidieR, the `add_skimr_metadata()` function attaches standard [skimr](https://docs.ropensci.org/skimr/) metrics to the [metadata tibbles](#metadata-tibble) of the [supertibble](#supertibble) output.
