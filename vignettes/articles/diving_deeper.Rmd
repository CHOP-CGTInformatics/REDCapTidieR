...
REDCap returns export data in a format where column types must be inferred. In sparse projects (for example, columns with many missing values), automatic parsing may occasionally infer an unexpected R type for a field.

`read_redcap()` checks parsed column types against REDCap metadata and warns if there is a likely mismatch. This warning is intended to flag cases where a field may have been parsed in a way that could affect downstream analysis.
...
When this occurs, use `col_types` to explicitly enforce expected parsing behavior:
