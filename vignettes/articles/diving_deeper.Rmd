...
ℹ Set `enable_mixed_structure` to `TRUE` to override. See Mixed Structure Instruments for more information.
```

This is because such a design inherently goes against [tidy](glossary.html#tidy) data principles. 

However, as of REDCapTidieR v1.1.0 it is now possible to override this behavior by setting `enable_mixed_structure` in `read_redcap()` to `TRUE`. When enabled, nonrepeating variants of mixed structure instruments will be treated as repeating instruments with a single repeating instance. 
