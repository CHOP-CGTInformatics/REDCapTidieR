...
Let's take a look at the data tibbles derived from the Superhero database and contrast with the block matrix. Consider `heroes_information`, which contains data from a **nonrepeating** instrument. This tibble is much smaller than the block matrix at 734 rows by 12 columns, and there are no `NA`s. The data tibble's name `heroes_information` is descriptive. REDCapTidieR knows the names of all instruments because in addition to querying the project's data it also queries the project's metadata. Each entry has the same observational unit, one **superhero**, identified by its `record_id`. A number of data columns follow, starting with `name`, `gender`, `eye_color`, `race`, etc. These columns are in the same order as on the REDCap instrument. The last column of a REDCapTidieR data tibble is always `form_status_complete` which indicates whether the instrument has been marked as completed.

```{r}
library(REDCapTidieR)

import_redcap(redcap_uri, superheroes_token) |>
  bind_tibbles()
...
longitudinal <- import_redcap(redcap_uri, longitudinal_token)
...
![REDCap field editor showing the choices of a single-answer field](field-editor-ae-grade.png)

</center>

The fact that all makes it possible to discover all of the different options for a specific field even if not all of those options are represented in the data. For example, even though we have only Grade 1 and 2 adverse events in the database, we can see that there are 5 possible grades.
...
![REDCap field editor showing the choices of a multi-answer field](field-editor-contributing-factors.png)
...
By default, `import_redcap()` returns these fields if the instrument is used as a survey. For the `survey` instrument, we get `survey_timestamp` and `redcap_survey_identifier`.

```{r, warning = FALSE}
survey <- import_redcap(redcap_uri, survey_token) |>
...
2.  The first column is always the record ID column which is derived from the record ID field. By default, REDCap names the record ID field `record_id`, but this can be changed, and REDCapTidieR is smart enough to handle this. For example, if the record ID field was renamed to `subject_id` then the record ID column of each data tibble would be `subject_id`.
3.  Additional identifying columns may follow the record ID column, depending on the structure of the instrument and the structure of the project (see the table above).
4.  After the identifying columns, data columns appear in the same order as in the REDCap instrument. True/false type fields are converted to logical columns. Single-answer categorical fields are converted to factor columns. Multi-answer checkbox fields are expanded to a set of logical columns, one for each choice.
5.  If the instrument is used as a survey, the survey timestamp and survey identifier columns will appear next.
