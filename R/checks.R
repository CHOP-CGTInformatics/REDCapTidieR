#' @title
#' Check for invalid API token supplied

#' @description
#' Check for when a user provides a valid 32 length hexadecimal that conforms to
#' checkmate checks, but the API key itself is not valid according to the
#' database user registry.
#'
#' @return
#' A helpful error message alerting the user of an invalid API token.
#'
#' @importFrom rlang .data caller_env
#' @importFrom cli cli_abort
#'
#' @param db_metadata The REDCap metadata output generated by \code{REDCapR::redcap_metadata_read()}
  ...
  if (db_metadata$success == FALSE & db_metadata$status_code == 403) {
    cli_abort(
      c("x" = "The REDCapR metadata export operation was not successful.",
        ...
        privileges or is incorrectly formed."),
      class = c("redcap_invalid_api", "REDCapTidieR_cond"),
      call = call
    )
  }
}
